...
def agent():
    return QLearningAgent(
        n_actions=N_ACTIONS, epsilon=1.0, epsilon_min=0.05, epsilon_decay=0.9
    )
...
class TestQLearningAgent:
    ...
    def test_update_changes_q_value(self, agent):
        obs = make_obs(60.0, 1.5)
        nobs = make_obs(58.0, 1.45)
        a = 10
    ...
    def test_terminal_ignores_next_state(self, agent):
        obs = make_obs(88.0, 1.8)
        nobs = make_obs(30.0, 1.1)
        a = 0
        t, p = agent._discretise(obs)
    ...
    def test_exploitation_deterministic(self, agent):
        ...
        t, p = agent._discretise(obs)
        agent.q_table[t, p, :] = 0.0
        agent.q_table[t, p, 42] = 999.0
        for _ in range(10):
            assert agent.select_action(obs) == 42
    ...
    def test_save_load_roundtrip(self, agent):
        ...
        try:
            agent.save(path)
            a2 = QLearningAgent(n_actions=N_ACTIONS)
            a2.load(path)
...
class TestReplayBuffer:
    ...
    def test_push_and_len(self):
        ...
        for i in range(10):
            buf.push(obs, i % 5, float(i), obs, False)
        assert len(buf) == 10
