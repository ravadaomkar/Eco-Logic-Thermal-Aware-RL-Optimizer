...
class TestDataCenterEnv:
    ...
    def test_truncation_at_max_steps(self):
        e = DataCenterEnv(max_steps=5); e.reset(seed=5); done = False; steps = 0
        while not done:
    ...
    def test_thermal_runaway_terminates(self):
        e = DataCenterEnv(cooling_budget=0.0, max_steps=1000); e.reset(seed=6)
        for r in e.racks: r.temp = 85.1
        _, _, term, _, _ = e.step(0)
        assert term

    def test_info_keys(self, env):
        env.reset(seed=7); _, _, _, _, info = env.step(0)
    ...
    def test_rack_temps_bounded(self, env):
        ...
        for r in env.racks: assert 28.0 <= r.temp <= 90.0
